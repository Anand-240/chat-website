...
const srcFor = (u) => /^https?:\/\//i.test(u || "") ? u : `${API_BASE_URL}${u || ""}`;
  ...
  return (
    <>
      <div className={`flex ${mine ? "justify-end" : "justify-start"} mb-3 px-3`}>
        <div className={`max-w-[85%] rounded-2xl px-0 pb-2 pt-0 shadow-sm overflow-hidden ${mine ? "bg-linear-to-tr from-(--accent1) to-(--accent2) text-white" : "bg-white border border-(--border)"}`}>
          {imgSrc && (
            <img src={imgSrc} alt="" className={`block w-full h-auto ${msg.text ? "rounded-b-none" : ""} rounded-2xl max-w-[640px]`} onClick={() => setOpen(true)} />
          )}
          {msg.text ? (
            ...
            <div className={`px-4 pt-2 ${mine ? "text-white/80" : "text-[#6b7280]"}`}>
              <div className="text-[10px]">{new Date(msg.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</div>
            </div>
          )}
        </div>
      </div>
      {open && imgSrc && (
        <div className="fixed inset-0 z-50 bg-black/80 grid place-items-center p-4" onClick={() => setOpen(false)}>
          ...
          <button className="absolute top-4 right-4 h-10 px-4 rounded-full bg-white/90 text-black text-sm" onClick={() => setOpen(false)}>Close</button>
        </div>
      )}
    </>
  );
