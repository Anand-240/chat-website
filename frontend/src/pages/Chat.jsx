...
export default function Chat({ onLogout }) {
  ...
  useEffect(() => {
    ...
    const onNew = (msg) => {
      ...
      const chattingThisThread =
        String(other) === String(receiverId) || (!isOid(receiverId) && String(msg.sender) === me);

      if (chattingThisThread) {
        setMessages((prev) => [...prev, msg]);
        if (!isOid(receiverId)) setReceiverId(other);
      }
      ...
      setConvos((prev) => {
        ...
        if (idx < 0) {
          p.unshift({ id: other, preview, time: now, unread: chattingThisThread ? 0 : 1 });
          return p;
        }
        const row = {
          ...p[idx],
          preview,
          time: now,
          unread: chattingThisThread ? 0 : (p[idx].unread || 0) + (String(msg.sender) !== me ? 1 : 0),
        };
        p.splice(idx, 1);
        p.unshift(row);
        return p;
        ...
        <div className="px-6 py-4 border-b border-(--border) flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              onKeyDown={handleKey}
            />
            <button onClick={applyReceiver} className="rounded-xl px-4 py-2 bg-(--primary) text-white text-sm font-medium">
              Start
            </button>
            <button onClick={() => { logout(); onLogout && onLogout(); }} className="rounded-xl px-4 py-2 text-sm bg-[#f4f5f9]">
              Logout
            </button>
          </div>
        </div>
